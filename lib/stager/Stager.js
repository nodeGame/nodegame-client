...
debugger
    ...
    Stager.prototype.endBlock = function(options) {
        ...
        // If it is a stage block do nothing (already added to block 0).
        if (block.id && block.id.indexOf('AAA') === -1) {
            ...
            if (block.id !== BLOCK_DEFAULT &&
                (!block.id || block.id.indexOf(BLOCK_ENCLOSING) === -1)) {

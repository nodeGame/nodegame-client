(function (exports, node) {
	...
	PlayerList.prototype.updatePlayer = function (id, player) {
	...
	// TODO: improve
	// Returns true if all the players are on the same state
	// and they are all GameState = DONE.
	// If strict is TRUE, also not initialized players are taken into account
	PlayerList.prototype.isStateDone = function(state, strict) {
		
		// console.log('1--------> ' + state);
		
		// Check whether a state variable is passed
		// if not try to use the node.state
		...
		var total = (strict) ? this.length : this.actives(); 
...
//		console.log(total + ' ' + sum);
	...
	PlayerList.prototype.actives = function (state) {
		var result = 0;
	...
	PlayerList.prototype.checkState = function (state, strict) {
		if (this.isStateDone(state, strict)) {
			node.emit('STATEDONE');
		}
	};
	...
	function Player (pl) {
		...
		// Adding non-default properties
		for (var key in pl) {
			if (pl.hasOwnProperty(key)) {
				// For security reasons, 
				// it is not allowed to add functions
				if ('function' !== typeof pl[key]) {
					// We don't overwrite previously set variables

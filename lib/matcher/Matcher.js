     ...
     */
     function pairMatcher(alg, n, options) {
         ...
         if ('undefined' !== typeof options.cycle) {
             cycle = options.cycle;
             if (cycle !== 'invert' && cycle !== 'mirror' &&
                 cycle !== 'shuffle') {

                 throw new Error('pairMatcher.' + alg + ': options.cycle ' +
                                 'must be equal to "invert", "mirror", ' +
                                 '"shuffle" or undefined . Found: ' +
                                 options.rounds);
             }
         ...
         for ( ; ++i < lenI ; ) {
             ...
             if (cycle) {
                 if (cycle === 'mirror') cycleI = (roundsLimit*2) -i -1;
                 else cycleI = i+roundsLimit;

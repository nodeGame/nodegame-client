...
(function(exports, node) {
    ...
    function hasOrGetNext(m, get, x, y) {
        ...
        if ('undefined' === typeof x && 'undefined' !== typeof y) {
            throw new Error('Matcher.' + m + ': cannot specify y without x.');
        }
        // Check if there is any match yet.
        if (!J.isArray(this.resolvedMatches) || !this.resolvedMatches.length) {
        ...
        if ('undefined' === typeof x) {
            ...
            if (get) {
                ...
                return !!this.resolvedMatches[x][0];
            }
        }
        ...
        if (x > nRows) {
            if (get) {
                this.x = nRows;
                this.y = this.resolvedMatches[0].length;
                return null;
            }
            else {
                return false;
        ...
        if (get) return this.resolvedMatches[x][y];
        else return true;
    }

...
(function(exports, parent) {
     ...
     *
     * @param {array} A match array containing two valid ids, or 1 id and
     *   one id and a 'miss-id'
     *
     * @return {string|null} The role currently hold, or null
     *    if the id is not found
     *
     * @see Roler.map
     */
    Roler.prototype.rolify = function(match, x, y, missId) {
        var roles, len;
        var id1, id2, soloIdx;
        len = match.length;
        if (!len) {
     ...
     * @see Roler.rolesMap
     */
    Roler.prototype.rolifyAll = function(matches, missId) {
        ...
        for ( ; ++i < len ; ) {
            ...
            for ( ; ++j < lenJ ; ) {
                rolesMap[i][j] = this.rolify(row[j], i, j, missId);
            }
        }

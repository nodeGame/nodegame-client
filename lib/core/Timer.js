...
(function(exports, parent) {
    ...
    /**
     * ## Timer.parseInputToTime
     *
     * Resolves an unknown value to a valid time interval (>=0)
     *
     ...
     *   the function for the error messsage. Default: parseInputToTime
     *
     * @param {number} The parsed value
     */
    Timer.prototype.parseInputToTime = function(name, value, methodName) {
        var typeofValue;
        ...
        if ('number' !== typeof num || num < 0) {
            throw new Error('Timer.' + methodName || 'parseInputToTime' +
                            ': ' + name + ' must be number >= 0.');
        }
    ...
    GameTimer.prototype.init = function(options) {
        ...
        if (options) {
            ...
            if ('undefined' !== typeof options.milliseconds) {
                this.milliseconds = this.parseInput('milliseconds',
                                                    options.milliseconds);
            }
            if ('undefined' !== typeof options.udpate) {
                this.update = this.parseInput('update',
                                              options.update);
            }
            else {
                // TODO: check what default is better.
...
//                }
     ...
     * makes it an object with property 'milliseconds'.
     *
     * If property 'timeup' is not defiend, it looks it up in the game plot.
     *

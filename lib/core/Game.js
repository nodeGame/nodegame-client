...
//        });
         ...
         * next step.
         *
         * Unlike `Game.plChangeHandler` this method does accept parameters
         * and does not execute any callback, but just returns TRUE/FALSE.
         *
         * @return {boolean} TRUE if all checks are passed
         *
         * @see Game.shouldStep
         * @see Game.gotoStep
         * @see Game.plChangeHandler
         */
         ...
         * of properties `min|max|exactPlayers` found in the Stager.
         *
         * Unlike `Game.checkPlistSize` this method requires input
         ...
         *
         * @see Game.min|max|exactPlayers
         * @see Game.min|max|exactPlayersCbCalled
         * @see Game.gotoStep
         * @see Game.checkPlistSize
         */
        this.plChangeHandler = function(op, obj) { return true };
         ...
         * ### Game.maxPlayers
         *
         * The max-players threshold for current round, if set.
         */
         ...
         * ### Game.minPlayers
         *
         * The min-players threshold for current round, if set.
         */
         ...
         * ### Game.exactPlayers
         *
         * The exact-players threshold for current round, if set.
         */
        this.exactPlayers = null;

        /**
         * ### Game.maxPlayersCbCalled
         *
         ...
         * @see Game.gotoStep
         */
        this.maxPlayersCbCalled = false;

        /**
         * ### Game.exactPlayersCbCalled
         *
         ...
         * @see Game.gotoStep
         */
        this.exactPlayersCbCalled = false;
...
//             if (options.willBeDone) this.willBeDone = true;
            ...
            if (property) {
                ...
                if (minThreshold === '*') {
                    throw new Error('Game.gotoStep: maxPlayers cannot be' +
                                    'defined if minPlayers is "*"');
                }

                if (maxThreshold <= minThreshold) {
                    throw new Error('Game.gotoStep: maxPlayers must be ' +
                                    'greater than minPlayers: ' +
                                    maxThreshold + '<=' + minThreshold);
                }

                // Added.
                this.maxPlayers = maxThreshold;
            ...
            if (property) {
                ...
                // Added.
                this.exactPlayers = minThreshold;
...
// Register event handler.
                ...
                // node.events.ee.step.on('UPDATED_PLIST', handler);

                // was:
                node.events.ee.step.on('in.say.PCONNECT', function(p) {
                    handler('pconnect', p);
                });
                node.events.ee.step.on('in.say.PDISCONNECT', function(p) {
                    handler('pdisconnect', p);
                });
                ...
                this.checkPlistSize = function() {
                    ...
                    limit = node.game.minPlayers;
                    if (limit && limit !== '*' && nPlayers < limit) {
                        return false;
                    }

                    limit = node.game.maxPlayers;
                    if (limit && limit !== '*' && nPlayers > limit) {
                        return false;
                    }

                    limit = node.game.exactPlayers;
                    if (limit && limit !== '*' && nPlayers !== limit) {
                        return false;
                    }
...
//                this.exactPlayersCbCalled = false;
    ...
    function checkMinMaxExactParams(name, property, node) {
        ...
        if ('number' === typeof property) {
            ...
            throw new TypeError('Game.gotoStep: ' + name + 'Players property' +

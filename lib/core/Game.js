 ...
 * 
 * Important: it does not use `Game.publishStage` because that is
...
Game.prototype.execStage = function(stage) {
	...
	this.updateStageStage('LOADING');
	
			
	try {
		res = cb.call(node.game);
		this.updateStageStage('LOADED');
...
Game.prototype.updateStageStage = function (state) {
	this.stageStage = state;
	//this.publishUpdate();
};

...
GameLoop.prototype.next = function(curStage) {
	...
	if (flexibleMode) {
		...
		curStage = this.normalizeGameStage(curStage, true);
		if (!curStage) {
			node.warn('next received invalid game stage: ' + curStage);
			return null;
		}
		...
		// Normalize game stage:
		curStage = this.normalizeGameStage(curStage, false);
		if (!curStage) {
			node.warn('next received invalid game stage: ' + curStage);
			return null;
		}
 ...
 *
 * Returns GameStage with numerical stage/step fields
 *
 * Leaves argument unmodified.
 *
 * @param {object} gameStage GameStage possibly with strings in stage/step
 * @param {boolean} normalizeStage Whether to normalize or keep the stage field
 *
 * @returns {object} GameStage with numerical stage/step on success, null on error
 *
 * @api private
 */
GameLoop.prototype.normalizeGameStage = function(gameStage, flexibleMode) {
	var stageObj;
	var seqIdx, seqObj;
	var stageNo = gameStage.stage;
	var stepNo  = gameStage.step;
	...
	if (!flexibleMode) {
		...
		// Get stage object:
		stageObj = this.plot.stages[this.plot.sequence[stageNo - 1].id];
	}
	else {
		// Get stage object:
		stageObj = this.plot.stages[gameStage.stage];
	}

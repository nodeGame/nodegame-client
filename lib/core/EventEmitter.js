 ...
 */
EventEmitter.prototype.emit = function(type) { // Up to 3 parameters
 ...
 */
	...
	switch (len) {
	    // fast cases
	case 2:
	    this[groupName] = this.ee[arguments[1]];
	    break;
	case 3:
	    this[groupName] = {
		emit: function() {
		    this.ee[arguments[1]].emit.apply(arguments);
		    this.ee[arguments[2]].emit.apply(arguments);
		},
		on: this.ee[arguments[1]].on,
		once: this.ee[arguments[1]].once,
		clear: function() {
		    this.ee[arguments[1]].clear();
		    this.ee[arguments[2]].clear();
		},
		remove: function() {
		    this.ee[arguments[1]].remove.apply(arguments);
		    this.ee[arguments[2]].remove.apply(arguments);
		},
		printAll: function() {
		    this.ee[arguments[1]].printAll();
		    this.ee[arguments[2]].printAll();
		}
	    };
	    break;
	default:
		...
		emit: function() {
		    for (i = 1; i < len; i++) {
			this.ee[arguments[i]].emit.apply(arguments);
		    }
		},
		on: this.ee[arguments[1]].on,
		once: this.ee[arguments[1]].once,
		clear: function() {
		    for (i = 1; i < len; i++) {
			this.ee[arguments[i]].clear();
		    }
		   
		},
		remove: function() {
		     for (i = 1; i < len; i++) {
			this.ee[arguments[i]].remove.apply(arguments);
		     }
		},
		printAll: function() {
		    for (i = 1; i < len; i++) {
			this.ee[arguments[i]].printAll();
		    }
		}
	    };
	}
	...
	for (i in this.ee) {
	    if (this.ee.hasOwnProperty(i)) {
		this.ee[i].emit.apply(arguments);
	    }
	}
    };

...
(function(exports, parent) {
    ...
    Stager.prototype.firstStep = function(step) {
        this.extendStep(this.currentType,step);
    }

    Stager.prototype.nextStep = function(step, positions, stage) {
        this.addStep(step);
        this.getCurrentBlock().add(step, positions);
    ...
    Stager.prototype.init = function() {
        this.sequence = [];
        this.beginBlock();
        this.rootBlocks["__default"] = this.getCurrentBlock();
        return this;
    };
    ...
    Stager.prototype.next = function(id, positions) {
        var i,I;
        var type = this.getCurrentBlock().type;
        var stageName = handleAlias(this, id);
        if (stageName === null) {
            throw new Error('Stager.next: invalid stage name received.');
        }
        // Trying to add a stage to a step block
        if (type !== "__default") {
            for (i = 0; i < this.unfinishedBlocks.length; ++i) {
                if (this.unfinishedBlocks[i].type === type) {
                    this.unfinishedBlocks.splice(i,1);
                }
            }
            I = this.blocks.indexOf(this.rootBlocks[type]);
            for (i = I; i < this.blocks.length; ++i) {
                this.blocks[i].type = "__default";
            }
            this.currentType = "__default";
        ...
        this.currentType = id;
        this.beginBlock();
        this.rootBlocks[this.currentType] = this.getCurrentBlock();
    ...
    Stager.prototype.finalize = function() {
        ...
        for (type in this.rootBlocks) {
            if (this.rootBlocks.hasOwnProperty(type)) {
                outermostBlock = this.rootBlocks[type];
                ...
                while (!!currentItem) {
                    if (type === "__default") {
                        ...
                        this.stages[type].steps.push(currentItem.id);
                    }
                    currentItem = outermostBlock.next();
                }
            }
     ...
     *
     * @param {string} positions Optional. Positions within the enclosing Block
     *      that this block can occupy.
     */
    function Block(positions, options) {

        options = options || {};

        this.type = options.type || "__default";
        /**
         * ### Block.positions
         *
         * Positions within the enclosing Block that this block can occupy
         */
         ...
         *
         * Items that have not yet been assigned a position within this block
         */
     ...
     *      Setting this parameter to "linear" or undefined adds the item to the
     *      next free n-th position where this is the n-th call to add.
     */
     ...
     * ## Block.finalize
     *
     * Processes all unfinished entries and assigns each to a definite position
     */
     ...
     * If the next item is not a Block it is returned, otherwise next is called
     *  recursively.
     */
    ...
    Stager.prototype.beginBlock = function(positions, options) {
        ...
        var block = new Block(positions, options);
     ...
     *
     * param {object} options Optional If options.finalize is set, the block
     *      gets finalized.
     */
    Stager.prototype.endBlock = function(options) {
        ...
        options = options || {};
        if (currentBlock && block.type === currentBlock.type) {

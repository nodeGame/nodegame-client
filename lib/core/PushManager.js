...
(function(exports, parent) {
    ...
    PushManager.prototype.startTimer = function(conf) {
        ...
        conf = conf || {};
        if ('undefined' !== typeof conf.timer) {
            timer = this.node.timer.parseInputToTime(conf.timer);
        }
        else {
            timer = 0;
        }
        if ('undefined' !== typeof conf.offset) {
            offset = this.node.timer.parseInputToTime(conf.offset);
        }
        else {
        ...
        node.silly('push-manager creating timer: ' + timer +

...
node.game.start = function() {
	var step;
	
	step = node.game.plot.next(); // returns the first stage if no argument is passed
	
	node.game.execStep(step);	
};

node.game.execStep = function(step) {
	if (!step) {
		// throws Error
	}
...
// Should take into account four level: Stage, Step, step_round, stage_round
...
function addMultiple(steps, pos) {
	var res = true, i;
	if (!steps.length) {
		node.warn('Cannot add empty array of steps');
		return false;
	}
	
	for (i = 0; i < steps.length; i++) {
		res = res && this.add(steps[i], pos+i);
	}
	return res;
}
 ...
 * 
 * @param {array|object} steps A GameStep object or an array of steps
 * @param {number} Optional. The ordinal position for inserting in array of 
 * 	steps. Negative values specify the position from the end of the array.
 * 	Defaults, adds at end of the array.
 * 	
 * @return {boolean} TRUE if the operation is completely successful
 *  
 */
Stager.prototype.add = function(step, pos) {
	
	if (J.isArray(step)) {
		return addMultiple.call(this, step, pos);
	}
	
	step = this.create(step);
	
	if (!step) return false;
	
	pos = pos || this.steps.length;
...
Stager.prototype.addStep = function(step, pos) {
	
	if (J.isArray(step)) {
		return addMultipleSteps.call(this, step, pos);
	}
...
Stager.prototype.createStage = function(name, steps, rounds, options) {
	this.stages[name] 
}
...
Stager.prototype.create = function(step) {
	if (!step || J.isEmpty(step)) {
		node.warn('Cannot create empty step.');
		return;
	}
	if ('string' !== typeof step.name) {
		node.warn('Invalid step name.');
		return;
	}
	if (step.name.trim() === '') {
		node.warn('Step name cannot be empty.');
		return;
	}
	
	if ('function' !== typeof step.cb) {
		node.warn('A step must have one valid callback.');
		return;
	}
 ...
 */
Stager.prototype.getByGameState = function (gameState) {
	gameState = new GameState(gameState);
...
Stager.prototype.createStage = function(stage) {
